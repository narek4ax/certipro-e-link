...
$enable_address_management = CPLINK::isset_return($cp_modules_cf, 'enable_address_management');
$enable_billing_address_management = CPLINK::isset_return($cp_modules_cf, 'enable_billing_address_management');
$enable_shipping_address_management = CPLINK::isset_return($cp_modules_cf, 'enable_shipping_address_management');
...
if ( ! $type ) {
	if ( $wc_address_book->get_wcab_option( 'billing_enable' ) === true ) {
		...
		if ( ! empty( $woo_address_book_billing_address ) ) {
			...
			<div class="address_book billing_address_book">
				<header>
					<h3><?php esc_html_e( 'Billing Address Book', 'woo-address-book' ); ?></h3>
					<?php
                    if($enable_address_management && $enable_billing_address_management) {
				...
				<div class="cp_address_book addresses address-book">
						...
						if ( $woo_address_book_formatted_address ) {
							...
							<div class="wc-address-book-address">
								<div class="wc-address-book-meta">
                                    <?php if($enable_address_management && $enable_billing_address_management) {?>
	...
	if ( $wc_address_book->get_wcab_option( 'shipping_enable' ) === true ) {
		...
		if ( ! empty( $woo_address_book_shipping_address ) ) {
			...
			<div class="address_book shipping_address_book">

				<header>
					<h3><?php esc_html_e( 'Shipping Address Book', 'woo-address-book' ); ?></h3>
					<?php
                    if($enable_address_management && $enable_shipping_address_management) {
                        // Add link/button to the my accounts page for adding addresses.
...
//					echo '<div class="col2-set addresses address-book">';
					echo '<div class="cp_address_book addresses address-book">';
				}
				...
				foreach ( $woo_address_book_shipping_address_book as $woo_address_book_name => $woo_address_book_fields ) {
					...
					if ( $woo_address_book_formatted_address ) {
						...
						<div class="wc-address-book-address">
							<div class="wc-address-book-meta">
                                <?php if($enable_address_management && $enable_shipping_address_management) {?>
